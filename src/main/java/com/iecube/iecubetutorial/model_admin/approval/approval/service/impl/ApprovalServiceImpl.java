...
public class ApprovalServiceImpl implements ApprovalService {
    ...
    private void handlePricingApproval(Approval approval) {
        try{
            ...
            PriceUnit rmbToP = new PriceUnit();
            rmbToP.setType("RMBTOPOINTS");
            rmbToP.setSource(1);
            rmbToP.setSourceUnits("RMB");
            rmbToP.setSourceUnitsCn("元");
            rmbToP.setTarget(priceChangeQo.getHowPointsPerRMB());
            rmbToP.setTargetUnits("POINTS");
            rmbToP.setTargetUnitsCn("积分");
            rmbToP.setActive(1);
            rmbToP.setRemoved(0);
            rmbToP.setCreateTime(Instant.now());
            rmbToP.setCreator(approval.getCreator());
            rmbToP.setLastOperateTime(Instant.now());
            rmbToP.setLastOperator(priceChangeQo.getApprover());
            priceUnitMapper.createPrice(rmbToP);
            PriceUnit PointToToken = new PriceUnit();
            PointToToken.setType("POINTTOTOKENS");
            PointToToken.setSource(1);
            PointToToken.setSourceUnits("POINTS");
            PointToToken.setSourceUnitsCn("积分");
            PointToToken.setTarget(priceChangeQo.getHowTokensPerPoint());
            PointToToken.setTargetUnits("TOKENS");
            PointToToken.setTargetUnitsCn("token");
            PointToToken.setActive(1);
            PointToToken.setRemoved(0);
            PointToToken.setCreateTime(Instant.now());
            PointToToken.setCreator(approval.getCreator());
            PointToToken.setLastOperateTime(Instant.now());
            PointToToken.setLastOperator(priceChangeQo.getApprover());
            priceUnitMapper.createPrice(PointToToken);
